...
def checkAliases(aliases,text,texts,index):
    status = 0;
    bottomRightX = texts[index].bounding_poly.vertices[0].x;
    bottomRightY = texts[index].bounding_poly.vertices[0].y;
	                ...
	                if checkAliasesResponse['status']:
	                    if(field[2] == "table"):
	                        ...
	                        vertices = (['({},{})'.format(int(vertex.x), int(vertex.y))
	                                for vertex in text.bounding_poly.vertices]) 
	                        addToLogs("Drawing bounding box : " + field[0] + "  (" + requiredStatus + ") ", outputFolderPath);
	                        addToLogs(vertices ,outputFolderPath);
...
# debugLevel = logging.DEBUG;
# imagePath_ = "C:/Users/Sikha.P/PROJECTS/GoogleVision/Invoice - Subheaders.png";
...
# logging.basicConfig(filename=outputFolderPath_+'\\'+'log.txt', encoding='utf-8', level= debugLevel,format='%(asctime)s %(levelname)s:%(message)s' ) 
